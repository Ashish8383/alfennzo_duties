...
export default function CartScreen({ route }) {
  ...
  const FOOTER_H   = nzVertical(80) + Math.max(insets.bottom, nzVertical(16));
  ...
  const [seatErr,   setSeatErr]   = useState(''); // ← NEW
  ...
  // ── Validation — checks BOTH phone AND seat before allowing order ──────────
  const validateAndPlace = () => {
    ...
    if (!valid) return; // Stop here — show both errors to user
    doPlaceOrder();
  };
  ...
  // ── Button is disabled unless cart has items, phone is 10 digits, seat is
  //    selected, and no order is being created ────────────────────────────────
  const canPlace =
    cartItems.length > 0 &&
    /^\d{10}$/.test(phone.trim()) &&   // ← phone must be valid
    Boolean(seatInfo?.seat) &&          // ← seat must be selected
    !orderCreating;
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* ← NEW: inline error shown when user taps Place Order without a seat */}
              {seatErr ? (
          ...
          <View style={[scr.footer, { paddingBottom: footerPB }]}>
            {/* Hint shown when seat is missing (quick reminder near button) */}
            ...
            <TouchableOpacity
              ...
              disabled={orderCreating}   // only truly disabled while API is running
              activeOpacity={0.85}
            >
      ...
      {/* Seat picker modal — clears seatErr on confirm */}
      <SeatPickerSheet
        visible={showSeatPicker}
        seatingData={seatingData}
        onConfirm={info => { setSeatInfo(info); setSeatErr(''); }}   // ← clears error
...
const scr = StyleSheet.create({
  ...
  // Seat error — shown below prompt/card when validation fires
  ...
  placeBtn:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: nz(10), backgroundColor: PRIMARY, borderRadius: nz(14), paddingVertical: nzVertical(15), shadowColor: PRIMARY, shadowOpacity: 0.32, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 5 },
