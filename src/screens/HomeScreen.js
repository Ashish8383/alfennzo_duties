...
const skSt = StyleSheet.create({
  card: {
    backgroundColor: colors.white, borderRadius: nz(16), overflow: 'hidden',
    marginBottom: nzVertical(14), shadowColor: '#000',
    shadowOpacity: 0.06, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 3,
  },
  headerRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingHorizontal: nz(16), paddingTop: nzVertical(16), paddingBottom: nzVertical(12),
  },
...
// ─── Order Card ───────────────────────────────────────────────────────────────
const OrderCard = memo(({ order, actionLabel, actionIcon, actionLoading, onAction }) => {
...
const tabSt = StyleSheet.create({
  wrapper: {
    flexDirection: 'row', backgroundColor: '#E3F2ED', borderRadius: nz(32),
    padding: nz(4), marginHorizontal: nz(20), marginBottom: nzVertical(10), position: 'relative',
  },
  pill: {
    position: 'absolute', top: nz(4), bottom: nz(4), left: nz(4), borderRadius: nz(28),
...
const PendingScene = memo(({
  ...
  onRefresh, acceptingOrderIds, handleAcceptOrder, TAB_BAR_HEIGHT,
}) => {
  // ✅ Show skeleton on first load only (not on pull-to-refresh)
...
export default function HomeScreen({ navigation, route }) {
  ...
  // ✅ Derived directly from store — no local state, no flash on mount
  ...
  const prevOngoingCountRef = useRef(acceptedOrders.length);

  const switchTab = useCallback((index) => setTabIndex(index), []);

  useEffect(() => {
    const currentPendingCount = pendingOrders.length;
    const currentOngoingCount = acceptedOrders.length;
    const prevPending = prevPendingCountRef.current;
    const prevOngoing = prevOngoingCountRef.current;

    // Case 1: User was on Pending tab, all pending orders are gone, but there are ongoing orders
    if (tabIndex === 0 && currentPendingCount === 0 && currentOngoingCount > 0) {
      // Only switch if pending count actually decreased (order was accepted)
      if (prevPending > 0) {
    ...
    // Case 2: User was on Ongoing tab, all ongoing orders are gone, but there are pending orders
    else if (tabIndex === 1 && currentOngoingCount === 0 && currentPendingCount > 0) {
      // Only switch if ongoing count actually decreased (order was delivered)
      if (prevOngoing > 0) {
        requestAnimationFrame(() => {
          setTabIndex(0);
        });
    ...
    prevOngoingCountRef.current = currentOngoingCount;
  }, [pendingOrders.length, acceptedOrders.length, tabIndex]);
  ...
  // Optimize handleAcceptOrder for immediate UI feedback
  const handleAcceptOrder = useCallback(async (order) => {
    // ✅ Optimistic UI update - immediately switch tab if this was the last pending order
    const willBeLastPending = pendingOrders.length === 1;
    ...
    if (result.success) {
      ...
      // ✅ If this was the last pending order, ensure we switch to ongoing
      if (willBeLastPending && acceptedOrders.length > 0) {
        requestAnimationFrame(() => {
          setTabIndex(1);
        });
      }
  ...
  }, [acceptOrder, pendingOrders.length, acceptedOrders.length]);

  // Optimize handleMarkDelivered for immediate UI feedback
  const handleMarkDelivered = useCallback(async (order) => {
    // ✅ Optimistic UI update - immediately switch tab if this was the last ongoing order
    const willBeLastOngoing = acceptedOrders.length === 1;
    ...
    if (result.success) {
      ...
      // ✅ If this was the last ongoing order and there are pending orders, switch back
      if (willBeLastOngoing && pendingOrders.length > 0) {
        requestAnimationFrame(() => {
          setTabIndex(0);
        });
      }
  ...
  }, [markDelivered, acceptedOrders.length, pendingOrders.length]);
  ...
  const renderScene = useCallback(({ route: r }) => {
    ...
    pendingOrders, pendingOrdersLoading, acceptedOrders, acceptedOrdersLoading,
  ...
  const renderTabBar = useCallback((props) => (
    <View style={{ backgroundColor: colors.white, paddingTop: nzVertical(2), paddingBottom: nzVertical(4) }}>
      <CustomTabBar {...props} pendingCount={pendingOrders.length} ongoingCount={acceptedOrders.length} SW={SW} />
    </View>
  ), [pendingOrders.length, acceptedOrders.length, SW]);
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
        ...
        {!isOnDuty ? (
          ...
          <TabView
            ...
            lazy={false}
            style={{ flex: 1 }}
            swipeVelocityImpact={0.15}  // ✅ Lower value = smoother
            springConfig={{
              mass: 0.8,      // ✅ Lower mass = faster
              tension: 300,   // ✅ Higher tension = snappier
              friction: 25,   // ✅ Higher friction = less bounce
            }}
          />
        )}
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  switchThumb: {
    position: 'absolute', backgroundColor: colors.white, justifyContent: 'center', alignItems: 'center',
  ...
  actionBtn: {
    ...
    borderRadius: nz(12), paddingVertical: nzVertical(15), backgroundColor: colors.primary,
