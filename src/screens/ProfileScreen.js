...
const MENU_ITEMS = [
  ...
  { key: 'payoutHistory',label: 'Payout History',  icon: 'wallet-outline' },
...
function EditProfilePopup({ visible, user, onClose, onSave, isSaving }) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      setErrors({});
      Animated.parallel([
        Animated.spring(scaleAnim, { toValue: 1, useNativeDriver: true, bounciness: 8, speed: 14 }),
              ...
              <View style={popupStyles.fieldGroup}>
                ...
                <View style={[popupStyles.inputWrap, errors.dateOfBirth && popupStyles.inputError]}>
                  ...
                  <TextInput
                    style={popupStyles.input}
                    value={dateOfBirth}
                    onChangeText={(t) => { setDateOfBirth(maskDOB(t)); setErrors((e) => ({ ...e, dateOfBirth: undefined })); }}
...
function ContactSupportModal({ visible, onClose }) {
  ...
  const handleCall = () => {
    RNLinking.openURL(`tel:${SUPPORT_PHONE}`).catch(() => Alert.alert('Error', 'Unable to make call'));
  };

  const handleWhatsApp = () => {
    RNLinking.openURL(`whatsapp://send?phone=${SUPPORT_PHONE.replace('+', '')}`).catch(() => {
    ...
    Alert.alert('Logout', 'Are you sure you want to logout?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          if (result?.blocked) {
            Alert.alert('Cannot Logout', result.message, [{ text: 'OK', style: 'destructive' }]);
          }
        },
      },
    ]);
...
const contactStyles = StyleSheet.create({
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: nz(20) },
  modal: { backgroundColor: colors.white, borderRadius: nz(20), width: '100%', maxWidth: 400, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 20, elevation: 10 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: nz(20), paddingTop: nzVertical(20), paddingBottom: nzVertical(12) },
  title: { fontSize: rs(18), fontWeight: '700', color: colors.black },
  closeBtn: { padding: nz(4) },
  divider: { height: 1, backgroundColor: colors.border, marginBottom: nzVertical(16), marginHorizontal: nz(20) },
  content: { paddingHorizontal: nz(20), paddingBottom: nzVertical(20) },
  subtitle: { fontSize: rs(14), color: colors.textLight, marginBottom: nzVertical(20), textAlign: 'center' },
  option: { flexDirection: 'row', alignItems: 'center', paddingVertical: nzVertical(12), paddingHorizontal: nz(12), backgroundColor: '#F8F9FA', borderRadius: nz(12), marginBottom: nzVertical(10) },
  optionIcon: { width: nz(44), height: nz(44), borderRadius: nz(22), justifyContent: 'center', alignItems: 'center', marginRight: nz(12) },
  optionContent: { flex: 1 },
  optionTitle: { fontSize: rs(15), fontWeight: '600', color: colors.black, marginBottom: nzVertical(2) },
  optionSubtitle: { fontSize: rs(12), color: colors.textLight },
});

// ─── Popup Modal Styles ───────────────────────────────────────────────────────
const popupStyles = StyleSheet.create({
  overlay: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: nz(20) },
  popup: { backgroundColor: colors.white, borderRadius: nz(20), width: '100%', maxWidth: 400, maxHeight: '80%', shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.25, shadowRadius: 20, elevation: 10 },
  popupHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: nz(20), paddingTop: nzVertical(20), paddingBottom: nzVertical(12) },
  popupTitle: { fontSize: rs(18), fontWeight: '700', color: colors.black },
  closeBtn: { padding: nz(4) },
  divider: { height: 1, backgroundColor: colors.border, marginBottom: nzVertical(16), marginHorizontal: nz(20) },
  scrollContent: { paddingHorizontal: nz(20), paddingBottom: nzVertical(20) },
  fieldGroup: { marginBottom: nzVertical(18) },
  label: { fontSize: rs(13), fontWeight: '600', color: colors.text, marginBottom: nzVertical(8) },
  required: { color: '#E53935' },
  inputWrap: { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: colors.border, borderRadius: nz(12), paddingHorizontal: nz(12), height: nzVertical(50), backgroundColor: '#FAFAFA' },
  inputError: { borderColor: '#E53935', backgroundColor: '#FFF5F5' },
  inputIcon: { marginRight: nz(10) },
  calendarIconBtn: { marginRight: nz(10), padding: nz(4) },
  clearIconBtn: { padding: nz(4) },
  input: { flex: 1, fontSize: rs(14), color: colors.black, paddingVertical: 0 },
  errorText: { fontSize: rs(11), color: '#E53935', marginTop: nzVertical(4), marginLeft: nz(2) },
  hint: { fontSize: rs(11), color: colors.textLighter, marginTop: nzVertical(4), marginLeft: nz(2) },
  btnRow: { flexDirection: 'row', gap: nz(12), paddingHorizontal: nz(20), paddingTop: nzVertical(8), paddingBottom: nzVertical(20), borderTopWidth: 1, borderTopColor: colors.border },
  cancelBtn: { flex: 1, borderWidth: 1.5, borderColor: colors.border, borderRadius: nz(12), paddingVertical: nzVertical(14), alignItems: 'center' },
  cancelText: { fontSize: rs(14), fontWeight: '600', color: colors.textLight },
  saveBtn: { flex: 2, backgroundColor: colors.primary, borderRadius: nz(12), paddingVertical: nzVertical(14), alignItems: 'center', shadowColor: colors.primary, shadowOpacity: 0.3, shadowRadius: 6, elevation: 3 },
  saveText: { fontSize: rs(14), fontWeight: '700', color: colors.white },
});

// ─── Screen Styles ────────────────────────────────────────────────────────────
const styles = StyleSheet.create({
  safeArea: { flex: 1 },
  header: { paddingTop: nzVertical(8), paddingBottom: nzVertical(14), flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.border },
  headerTitle: { fontSize: rs(18), fontWeight: '700', color: colors.black },
  headerLoader: { position: 'absolute', right: nz(16) },
  body: { flex: 1, backgroundColor: '#F5F6FA' },
  scroll: { flex: 1 },
  scrollContent: { paddingHorizontal: nz(16), paddingTop: nzVertical(20), gap: nzVertical(14) },
  warningBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: (colors.warning || '#FB8C00') + '20', padding: nz(12), borderRadius: nz(12), gap: nz(12) },
  warningContent: { flex: 1 },
  warningTitle: { fontSize: rs(14), fontWeight: '600', color: colors.warning || '#FB8C00', marginBottom: nzVertical(2) },
  warningMessage: { fontSize: rs(12), color: colors.textLight },
  avatarCard: { backgroundColor: colors.white, borderRadius: nz(16), padding: nz(16), flexDirection: 'row', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  avatarCircle: { width: nz(62), height: nz(62), borderRadius: nz(31), backgroundColor: '#F5A623', overflow: 'hidden', alignItems: 'center', justifyContent: 'flex-end', marginRight: nz(14) },
  avatarHead: { width: nz(24), height: nz(24), borderRadius: nz(12), backgroundColor: '#5C3D11', marginBottom: nz(2) },
  avatarBody: { width: nz(40), height: nz(28), borderRadius: nz(20), backgroundColor: '#5C3D11' },
  avatarInfo: { flex: 1 },
  avatarName: { fontSize: rs(17), fontWeight: '700', color: colors.black, marginBottom: nzVertical(4) },
  phoneRow: { flexDirection: 'row', alignItems: 'center' },
  avatarPhone: { fontSize: rs(13), color: colors.textLight },
  editIconBtn: { marginLeft: nz(8) },
  editIconWrap: { width: nz(34), height: nz(34), borderRadius: nz(17), backgroundColor: colors.primary + '15', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: colors.primary + '30' },
  menuCard: { backgroundColor: colors.white, borderRadius: nz(16), paddingHorizontal: nz(16), shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  menuItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: nzVertical(15) },
  menuLeft: { flexDirection: 'row', alignItems: 'center', gap: nz(12) },
  menuIconWrap: { width: nz(36), height: nz(36), borderRadius: nz(10), backgroundColor: colors.primary + '12', justifyContent: 'center', alignItems: 'center' },
  menuLabel: { fontSize: rs(14), color: colors.black, fontWeight: '500' },
  menuDivider: { height: 1, backgroundColor: '#F0F0F0', marginLeft: nz(48) },
  logoutBtn: { backgroundColor: colors.white, borderRadius: nz(16), padding: nz(16), flexDirection: 'row', alignItems: 'center', gap: nz(12), shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 3, marginBottom: nzVertical(20) },
  logoutIconWrap: { width: nz(36), height: nz(36), borderRadius: nz(10), backgroundColor: '#FFEBEE', justifyContent: 'center', alignItems: 'center' },
