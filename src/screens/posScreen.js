...
const SkeletonMenuItem = () => (
  <View style={styles.skeletonItem}>
    ...
    <View style={styles.skeletonItemBody}>
      <SkeletonBox 
        width="70%" 
        height={nzVertical(14)} 
        borderRadius={nz(4)} 
        style={{ marginBottom: nzVertical(8) }} 
      />
      <SkeletonBox 
        width="45%" 
        height={nzVertical(12)} 
        borderRadius={nz(4)} 
        style={{ marginBottom: nzVertical(6) }} 
      />
      <SkeletonBox 
        width="35%" 
        height={nzVertical(11)} 
        borderRadius={nz(4)} 
        style={{ marginBottom: nzVertical(10) }} 
      />
...
const SkeletonSection = ({ showHeader = true, itemCount = 4 }) => (
  ...
  >
    ...
    <View style={styles.skeletonTabsContainer}>
      <ScrollView 
        ...
        contentContainerStyle={styles.skeletonTabsScroll}
      >
        {[90, 70, 85, 65, 95, 75].map((width, index) => (
          <SkeletonBox 
            key={index}
            width={nz(width)} 
            height={nzVertical(36)} 
            borderRadius={nz(20)}
            style={{ marginRight: nz(8) }}
          />
        ))}
      </ScrollView>
    </View>
...
export default function POSScreen() {
  ...
  const addToCart = useCallback(item => {
    ...
    setCart(prev => {
      ...
      if (isCombo) {
        entry.comboItemCount = item.comboItemCount || 
                              item.comboData?.ComboItems?.length || 
  ...
  const increaseQuantity = useCallback(id => {
    ...
    setCart(prev => 
      prev[id] ? { ...prev, [id]: { ...prev[id], quantity: prev[id].quantity + 1 } } : prev
    );
  }, [isOnDuty]);
          ...
          <View style={styles.header}>
            <View>
              ...
              <SkeletonBox 
                ...
                height={nzVertical(12)} 
                borderRadius={nz(4)} 
                style={{ marginTop: nzVertical(4) }} 
              />
            </View>
            <SkeletonBox width={nz(90)} height={nzVertical(36)} borderRadius={nz(20)} />
          </View>
          <POSMenuSkeleton />
        </SafeAreaView>
      </>
